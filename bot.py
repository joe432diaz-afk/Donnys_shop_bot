...
def build_order_buttons(order):
    buttons = []
    status = order[4]  # status
    oid = order[0]
    if status == "Paid":
        buttons.append([InlineKeyboardButton("✍️ Leave/Edit Review", callback_data=f"review_{oid}")])
        buttons.append([InlineKeyboardButton("👀 View Review", callback_data=f"viewreview_{oid}")])
...
async def my_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for order in orders:
        ...
        await q.message.reply_text(text, reply_markup=build_order_buttons(order))
...
async def review_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("review_"):
        ...
        if r:
            await q.message.reply_text(f"📝 Edit your review (current {r[0]}★):\n\n{r[1]}")
        else:
            await q.message.reply_text("📝 Write your review (1-5 stars, text). Format: 5 Excellent product!")
    elif data.startswith("viewreview_"):
        oid = int(data.replace("viewreview_", ""))
        cur.execute("SELECT stars, text FROM reviews WHERE order_id=? AND user_id=?", (oid, uid))
        r = cur.fetchone()
        if r:
            await q.message.reply_text(f"⭐ {r[0]} stars\n\n{r[1]}")
        else:
            await q.message.reply_text("❌ No review yet for this order.")
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("dispatch_"):
        ...
        return
    if data.startswith("admin_") or data=="admin_orders":
        # handle admin add product etc
        pass
...
print("✅ BOT RUNNING WITH REVIEWS")
app.run_polling()
